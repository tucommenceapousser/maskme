...
def calculate_face_rotation(face_landmarks, image_width, image_height):
    ...
    """
    # Prendre les coordonnées des yeux gauche et droit (indices 33 et 133)
...
def add_mask_to_face(image_path, mask_path, result_path):
    ...
    with mp_face_detection.FaceDetection(model_selection=1, min_detection_confidence=0.5) as face_detection, mp_face_mesh.FaceMesh(min_detection_confidence=0.5) as face_mesh:
        ...
        if results.detections:
            for detection in results.detections:
                ...
                if face_landmarks:
                    ...
                    # Redimensionner et appliquer la rotation au masque
                    resized_mask = cv2.resize(mask, (w, h))

                    # Rotation du masque
                    center = (w // 2, h // 2)
                    rotation_matrix = cv2.getRotationMatrix2D(center, angle, 1.0)
                    rotated_mask = cv2.warpAffine(resized_mask, rotation_matrix, (w, h))
